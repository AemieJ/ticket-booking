...
const calcLuxeryId = () => {
	...
	stateOfCustomer.luxeryId =
		(luxeryCount.popcorn > 0 ? luxeryCount.popcorn + "P" : "") +
		(luxeryCount.nachos > 0 ? luxeryCount.nachos + "N" : "") +
		(luxeryCount.chocolate > 0 ? luxeryCount.chocolate + "C" : "") +
		(luxeryCount.drink > 0 ? luxeryCount.drink + "D" : "") +
		uniqueUpdate.slice(uniqueUpdate.length - 4);
};
...
const popcornChoice = async () => {
	...
	if (+flavChoice["choose"] == 1) {
		...
		if (sizePop["ques"] == "S") {
			...
			popcornDetails = {
				size: "S",
				type_of_popcorn: "Salted",
				qty: luxeryCount.popcorn,
...
const drinkChoice = async () => {
	...
	if (+flavChoice["choose"] == 1) {
		...
		if (sizeDrink["ques"] == "S") {
			...
			drinkDetails = {
				size: "S",
				type_of_drink: "Coco Cola",
				qty: luxeryCount.drink,
				phone_no: stateOfCustomer.phoneNumber,
			};
		}
		if (sizeDrink["ques"] == "M") {
...
const nachosChoice = async () => {
	...
	if (+flavChoice["choose"] == 1) {
		...
		nachosDetails = {
			type_of_nachos: "Salted",
			qty: luxeryCount.nachos,
			phone_no: stateOfCustomer.phoneNumber,
		};
	}
...
const chocolateChoice = async () => {
	...
	if (+flavChoice["choose"] == 1) {
		...
		chocDetails = {
			type_of_chocolate: "Bounty",
			qty: luxeryCount.chocolate,
			phone_no: stateOfCustomer.phoneNumber,
		};
	}
	if (+flavChoice["choose"] == 2) {
...
const customerQuery = async () => {
	...
	if (initialAnswer["initial"] === "0") {
		...
		for (let _ = 0; _ < +seatAnswer["number"]; ++_) {
			...
			if (
				seatNumber["place"].indexOf("A") > -1 ||
				seatNumber["place"].indexOf("B") > -1 ||
				seatNumber["place"].indexOf("C") > -1
			) {
			...
			let ticket = calcTicketNo(
				genreMap[+movieAnswer["movie"]],
				movieAnswer["movie"],
				movieAnswer["movie"],
				seatNumber["place"]
			);
